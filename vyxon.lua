...
do
    function utility:Create(instance, properties, children)
        ...
        if object.Parent and object.Name ~= "" then
            local existing = object.Parent:FindFirstChild(object.Name)
    ...
    function utility:Pop(object, shrink)
        local existing = object:FindFirstChild("PopClone")
        if existing then
            existing:Destroy()
        end
    ...
    function utility:InitializeKeybind()
        ...
        input.InputBegan:Connect(function(key, proc)
            if self.keybinds[key.KeyCode] and not proc then
                for _, bind in pairs(self.keybinds[key.KeyCode]) do
                    bind()
                end
            end
        end)
...
do
    ...
    function library.new(title)
        local container = utility:Create("ScreenGui", {
            Name = title,
            Parent = game.CoreGui or game.ScreenGui
        }, {
...
-- local venyx = library.new("Vyxon", 5013109572)
